    ...
    getLineManagerDashboard: async function (req, res) {
        if (req.session.user && req.session.user.userRole === 'Line Manager') {
            try {
                ...
                const notifications = applications.map(applicant => ({
                    ...
                    content: applicant.applicantStatus === "P1 - Awaiting for Line Manager Action; HR PASSED"
                        ...
                        : `${applicant.firstName} ${applicant.lastName} has submitted an application.`,
                ...
                req.flash('errors', { dbError: 'An error occurred while loading notifications.' });
                return res.redirect('/linemanager/dashboard');
            }
        } else {
